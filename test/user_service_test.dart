...
void main() async {
  ...
  var _userService = MockUserServiceImpl();

  group('Test User Service', () {
    String path = "https://gorest.co.in/public/v1/users";

    test('returns a success state of [UserResponseModel] if successful',
        () async {
      when(_httpHelper.get(path)).thenAnswer(
        (_) => Future.value(ServiceState.success(responsePayload)),
      );

      when(_userService.getUsers()).thenAnswer(
        (_) => Future.value(ServiceState.success(responsePayload)),
      );
